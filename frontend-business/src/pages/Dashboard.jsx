...
export default function Dashboard() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title">Your ROI</span>
          <button className="btn btn-ghost btn-sm" onClick={() => navigate('/reports')}>Full reports</button>
        </div>
        <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {roi ? (
            <>
              <div className="stats-grid">
                <StatCard label="Extra profit" value={`£${roi.total_profit.toFixed(2)}`} accent />
                <StatCard label="Total revenue" value={`£${roi.total_revenue.toFixed(2)}`} />
                <StatCard label="Total cost" value={`£${roi.total_cost.toFixed(2)}`} />
                <StatCard label="Avg per redemption" value={roi.total_redemptions > 0 ? `£${roi.avg_profit_per_redemption.toFixed(2)}` : '£0.00'} sub={`${roi.total_redemptions} redeemed`} />
              </div>
              {roi.offers_with_pricing > 0 && (
                <div style={{ fontSize: '0.78rem', color: 'var(--c-text-muted)' }}>
                  Based on {roi.offers_with_pricing} offer{roi.offers_with_pricing !== 1 ? 's' : ''} with pricing data
                </div>
              )}
            </>
          ) : (
            <div style={{ textAlign: 'center', color: 'var(--c-text-muted)', padding: '24px 0', fontSize: '0.88rem' }}>
              Add cost and selling prices to your offers to see profit data here.
            </div>
          )}
          <div>
            <button type="button" className="btn btn-ghost btn-sm" onClick={() => setRoiExplain(!roiExplain)} style={{ fontSize: '0.78rem' }}>
              {roiExplain ? '▾' : '▸'} How is this calculated?
            </button>
            {roiExplain && (
              <div style={{ marginTop: 8, padding: '12px 16px', background: 'var(--c-bg-muted)', borderRadius: 'var(--r-md)', fontSize: '0.82rem', color: 'var(--c-text-muted)', lineHeight: 1.65 }}>
                <strong>Extra profit</strong> = (offer price − cost price) × number of redemptions. This shows the additional revenue generated by running deals on TapProve versus not running them at all.
              </div>
            )}
          </div>
        </div>
